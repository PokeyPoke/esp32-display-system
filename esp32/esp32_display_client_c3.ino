/*
  ESP32-C3 Display Client - Compatible Version
  Optimized for ESP32-C3 with proper watchdog handling
*/
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n\n=== ESP32-C3 Display Client v0.2.0 ===");
  
  // NO WATCHDOG INITIALIZATION - ESP32-C3 has it pre-configured
  ...
  // Clear WiFi settings for clean start
  Serial.println("Clearing stored WiFi credentials for clean setup");
  ...
  String mac = WiFi.macAddress();
  String apName = "ESP32-C3-Display-" + mac.substring(12);
  apName.replace(":", "");
...
void loop() {
  ...
  // Simple delay loop without watchdog resets
  for (int i = 0; i < waitSec * 10; i++) {
    delay(100);
    // Let ESP32-C3 handle its own watchdog
